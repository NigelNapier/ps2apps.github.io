		...
		if (!$resp -> is_valid) {
			...
			if (empty(zpanelx::$zerror)) {
				...
				if ($createBilling['code'] == "1" && $createInvoice['code'] == "1") {
					...
					zpanelx::sendmail(zpanelx::getConfig('error_email'), "Error creating billing", "The invoice have not been created for user: " . $username . "(" . $email . ") Error code:" . $createBilling['create_invoice']);
				}
			}
		}
	}
